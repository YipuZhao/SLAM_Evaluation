function [logStruct] = loadLogTUM_stereo(log_path, rn, seq_idx, logStruct, ln_head)
...
if fid ~= -1
  if ln_head > 0
    log_orb_slam = cell2mat(textscan(fid, '%f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f', ...
      'HeaderLines', ln_head));
  else
    log_orb_slam = cell2mat(textscan(fid, '%f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f'));
  end
...
logStruct.timeDirect{rn} = [];
%
logStruct.lmkTrackMotion{rn}    = log_orb_slam(:, 15);
logStruct.lmkTrackFrame{rn}     = log_orb_slam(:, 16);
logStruct.lmkTrackMap{rn}       = log_orb_slam(:, 17);
logStruct.lmkBA{rn}             = log_orb_slam(:, 18);
