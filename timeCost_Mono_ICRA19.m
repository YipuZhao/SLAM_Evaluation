...
for sn = 1:length(seq_list) % [1, 4, 10] %
    ...
    for gn=1:length(gf_slam_list)
      ...
      for rn = 1:round_num
        ...
        if tn <= baseline_orb
          ...
          %           [log_{gn, tn}] = loadLogTUM_hash([slam_path_list{tn} ...
          %             baseline_slam_list{table_index(tn)}], ...
          %             rn, seq_idx, log_{gn, tn}, seq_start_time(sn) * fps);
        elseif tn == 2
          disp(['Loading DSO log...'])
          [log_{gn, tn}] = loadLogDSO([slam_path_list{tn} ...
            baseline_slam_list{table_index(tn)}], ...
            rn, seq_idx, log_{gn, tn}, seq_start_time(sn) * fps);
        else
  ...
  if plot_total_time_stat
    ...
    for tn=plot_index % 1:length(slam_path_list)
      %
      for mn = 1:6
        err_all_rounds = [];
        %
  ...
  if plot_gf_time_trend
    ...
    hold on
    n = 1;
    for mn = 1:length(metric_type) % [2,3] %
      %     h = figure(100 + mn);
      %     clf
      %
      for tn=plot_index % [1:length(slam_path_list)]
...
if plot_total_time_stat
  % Plot Comparison Illustration
  % x = categorical({'ORB'; 'SVO'; 'DSO'; 'Random'; 'Longlive'; 'MIH - x/32'}');
  x = categorical({'ORB'; 'LDSO'; 'Random'; 'Longlive'; 'MIH - x/32'}');
  %   x = categorical({'1/32'; '4/32'; '8/32'; '16/32'; '32/32'; 'x/32'}');
  ...
  set(gca,'FontSize',12)
  %     set(h, 'Units', 'normalized', 'Position', [0,0,1,1]);
  export_fig(h, [save_path '/Bar_TotalTimeCost_Mono.png']); % , '-r 200');
