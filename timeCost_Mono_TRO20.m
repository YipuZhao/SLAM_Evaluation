  ...
  if plot_lmk_num
    ...
    for mn = 1:length(metric_type)
      ...
      for tn = [1:baseline_orb, baseline_num+1:length(slam_path_list)]
        err_summ{tn} = [];
        %
        for gn=table_index(end) % table_index(tn) % 4%1:length(gf_slam_list)
          err_all_rounds = [];
          %
          for rn=1:round_num
            %
...
if plot_total_time_stat
  ...
  %       'Longlive-ORB';
  %       }');
  %     x = reordercats(x, {'Longlive-ORB' 'Random-ORB' 'GF-ORB' 'ORB' 'DSO' 'SVO2' 'VINS-Mono' 'ROVIO'});
%
%     x = categorical({
%         'ORB';
%         'SVO2';
...
%         'Longlive-ORB';
%         }');
%       x = reordercats(x, {'Longlive-ORB' 'Random-ORB' 'GF-ORB' 'ORB' 'DSO' 'SVO2'});
%  
    x = categorical({'ORB'; 'SVO2'; 'DSO'; 'GF-ORB';}');
    x = reordercats(x, {'GF-ORB' 'ORB' 'DSO' 'SVO2' });
