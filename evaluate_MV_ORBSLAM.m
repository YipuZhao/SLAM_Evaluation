...
benchMark = 'EuRoC' % 'KITTI' % 'TUM' %
setParam
legend_arr = {'ORB-SLAM'; 'MaxVol-ORB-SLAM';}; % {'ORB-SLAM'; 'Bucket-ORB-SLAM';}; % {'ORB-SLAM'; 'GF-ORB-SLAM';}; %
...
for in = 1:length(rel_interval_list)
  for gn = 1:length(gf_slam_list)
    ...
    %%
    for sn = 1:length(seq_list)  % 2 % [2:4, 7] %
      ...
      for tn = 1:length(track_type)
        for rn= 1:round_num
          ...
          if fid ~= -1
            ...
            % frame
            track_orig = cell2mat(textscan(fid, '%f %f %f %f %f %f %f %f', 'HeaderLines', 1));
            fclose(fid);
          ...
          % load the max volume orb slam track
          fid = fopen([mv_slam_path gf_slam_prefix '_Round' num2str(rn) ...
            '/' seq_idx '_' track_type{tn} 'Trajectory.txt'], 'rt');
          if fid ~= -1
        ...
        if do_viz
          ...
          for dn=1:round_num
            if ~isempty(err_orig{tn}.seq{sn}.track_fit{dn}) && ...
              ...
              if length(err_orig{tn}.seq{sn}.abs_drift{dn}) + ...
                  length(err_gf{tn}.seq{sn}.abs_drift{dn}) > tlen_max
                dn_max = dn;
          ...
          if dn_max == -1
            ...
            for en = 1:length(err_type_list)
              ...
              %               h=figure();
              plotDriftSummary(plot3DTrack, sn, tn, track_type, track_loss_ratio, dn_max, round_num, fps, ...
              ...
              export_fig(h, [detail_save_path gf_slam_prefix ...
                '_' seq_idx '_' track_type{tn} ...
                '_MaxVol.png'], '-r 200');
