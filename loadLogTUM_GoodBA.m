function [logStruct] = loadLogTUM_GoodBA(log_path, rn, seq_idx, logStruct, ln_head)
...
if fid ~= -1
  ...
  if ln_head > 0
    log_ = cell2mat(textscan(fid, ...
      '%f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f', 'HeaderLines', ln_head));
  else
    log_ = cell2mat(textscan(fid, ...
...
%
logStruct.numPoseState{rn}      = log_(:, 17) * 6;
logStruct.numLmkState{rn}       = log_(:, 18) * 3;
